...
export const FormArea = ({ initialData }: FormAreaProps) => {
	...
	const onSubmit = async (values: z.infer<typeof createAreaSchema>) => {
		if (initialData) {
			try {
				const valuesUpdated = { ...values, state: values.state === 'Activo' ? true : false };
				await axios.patch(`/areas/${initialData._id}`, valuesUpdated);
				toast({
	...
	const onDelete = async () => {
		try {
			setIsDeleting(true);
			await axios.delete(`/areas/${initialData?._id}`);
			toast({
				title: 'Exito',
				description: 'Operación exitosa',
			});
